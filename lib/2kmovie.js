	...
	try {
		if (type == 'tv') {
			...
			const browser = await puppeteer.launch({
				headless: true,
				args: [`--disable-extensions-except=${ublock},${ghostery}`, `--load-extension=${ublock},${ghostery}` ]
			})
			const page = (await browser.pages())[0];
			await page.goto(real)	
			const vidlink = await page.waitForResponse((res) => {
				if(res.url().startsWith(`https://storage.googleapis.com`)){
					return res.url()
				}
			})
			await browser.close()
			...
			const browser = await puppeteer.launch({
				headless: true, 
				args: [`--disable-extensions-except=${ublock},${ghostery}`, `--load-extensions=${ublock},${ghostery}`]
			})
			const page = (await browser.pages())[0];
			await page.goto(real)	
			console.log(real)
